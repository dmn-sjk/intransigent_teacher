...
from utils.utils import clear_on_exit
...
_C.RESULTS_DEST = "results.yaml"

_C.WANDB = True
_C.WANDB_DIR = './wandb'

# whether to remove all the logs after experiment
_C.DEV = False
...
# Mean teacher momentum for EMA update
_C.LOSSTEST.LOSS = 'entropy' # entropy | slr | consistency | contrastive

_C.LOSSTEST.NORMONLY = False

_C.LOSSTEST.BNSTATS = 'test' # test | ema | src
 
_C.LOSSTEST.FRACRAND = 0.2
 
_C.LOSSTEST.RANDNUMITER = 0
...
def load_cfg_from_args(description="Config options."):
    ...
    cfg.SAVE_DIR = os.path.join(cfg.SAVE_DIR, cfg.CORRUPTION.DATASET, cfg.SETTING, cfg.MODEL.ADAPTATION, 
                                f"{cfg.RUN_NAME}_{current_time}")
    g_pathmgr.mkdirs(cfg.SAVE_DIR)
    cfg.LOG_TIME, cfg.LOG_DEST = current_time, "log.txt"

    _C.RESULTS_DEST = os.path.join(_C.SAVE_DIR, _C.RESULTS_DEST)
    # prepare empty result file
    with g_pathmgr.open(_C.RESULTS_DEST, "w"):
        pass
 
    if cfg.TEST.BATCH_SIZE == 10:
        cfg.OPTIM.LR = 0.15625 * cfg.OPTIM.LR
    
    job_type = f"{cfg.MODEL.ADAPTATION}_{cfg.RUN_NAME}"
    cfg.WANDB_DIR = os.path.join(cfg.WANDB_DIR, '_' + cfg.CORRUPTION.DATASET + job_type)

    if not os.path.exists(os.path.join(cfg.WANDB_DIR, 'wandb')):
        os.makedirs(os.path.join(cfg.WANDB_DIR, 'wandb'), exist_ok=True)
    
    cfg.freeze()
    
    if cfg.DEV:
        atexit.register(clear_on_exit, 
                        res_dir=cfg.SAVE_DIR,
                        log_wandb=cfg.WANDB, 
    ...
    if cfg.WANDB:
        ...
        wandb.init(
            mode='offline', # TODO: make it dependent on the arg
            settings=wandb.Settings(start_method='fork'),
            project='test_time_adaptation',
            name='seed' + str(cfg.RNG_SEED),
            job_type=job_type,
            group=cfg.CORRUPTION.DATASET,
            config=cfg,
            resume="allow",
            dir=cfg.WANDB_DIR)
...
def get_num_classes(dataset_name: str):
    dataset_name2num_classes = {"cifar10": 10, "cifar10_c": 10, "cifar100": 100,  "cifar100_c": 100,
                                ...
                                "domainnet126": 126, "ccc": 1000, "cifar10_1": 10, "vlcs": 7
                                }
...
def ckpt_path_to_domain_seq(ckpt_path: str):
    ...
    domain = ckpt_path_to_domain(ckpt_path)
...
def ckpt_path_to_domain(ckpt_path: str):
    return ckpt_path.replace('.pth', '').split(os.sep)[-1].split('_')[1]
